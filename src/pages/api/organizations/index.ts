...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createOrganization() {
    ...
    const body = { ...req.body };
    if (body?.guest_view?.length > 0) {
      const create_guest_view = body.guest_view;
      body.guest_view = {
        create: create_guest_view,
      };
    } else {
      delete body.guest_view;
    }
    ...
    if (body?.solar_plant?.length > 0) {
      const create_solar_plant = body.solar_plant;
      body.solar_plant = {
        create: create_solar_plant,
      };
    } else {
      delete body.solar_plant;
    }
