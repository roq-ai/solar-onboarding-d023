...
import { getPlantData } from 'apiSdk/plant-data';
...
import { GuestViewListPage as ShowCaseList } from 'pages/guest-views';
...
function DashboardPage() {
  ...
  const plantDataStat = useSWR<PaginatedInterface<PlantDataInterface>>(
    () => `/plant-data`,
    () => getPlantData({ limit: 1, offset: 0 }),
  );

  const solarPlantStat = useSWR<PaginatedInterface<SolarPlantInterface>>(
    () => `/solar-plants`,
    () => getSolarPlants({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Plant Data',
      result: plantDataStat?.data?.totalCount,
      isLoading: plantDataStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Solar Plants',
      result: solarPlantStat?.data?.totalCount,
      isLoading: solarPlantStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`guest-views`} passHref legacyBehavior>
                  <Button
