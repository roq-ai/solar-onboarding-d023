...
import { getPlantDataById, updatePlantDataById } from 'apiSdk/plant-data';
...
function PlantDataEditPage() {
  const router = useRouter();
  const id = router.query.id as string;

  const { data, error, isLoading, mutate } = useSWR<PlantDataInterface>(
    () => (id ? `/plant-data/${id}` : null),
    () => getPlantDataById(id),
  );
  ...
  const handleSubmit = async (values: PlantDataInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updatePlantDataById(id, values);
      mutate(updated);
      resetForm();
